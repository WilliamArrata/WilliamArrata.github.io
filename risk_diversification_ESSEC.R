...
w_a<-seq(from=0, to =1, length.out =300)
w_a<-as.matrix(cbind(w_a,rev(w_a)))                  #a range of weights for each asset in the portfolio
...
par(mar=c(4, 5, 4, 3))
plot(NA, las=1,xlab="standard deviation (%)", ylab="expected return (%)",xlim=xaxis, ylim=yaxis, las=1)
