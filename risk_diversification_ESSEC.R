...
legend("bottom", horiz=T,inset = c(0,-0.2),text.col=colv,pch=rep(NA,4),lty=c(rep(1,3),3),col=colv, bty="n", legend =
         expression(paste(rho[A][B]," = -1"),paste(rho[A][B]," = 1"),paste(rho[A][B]," = 0"),paste(rho[A][B]," = 0.25")))
...
frontier <- data.frame(do.call(rbind, pair_tot))
colnames(frontier) <- colnames(corner) <- c("stdev", "mean")
...
ggplot() +
  ...
  geom_point(data = corner, aes(stdev, mean), color="black", size=3) +
