...
require("pacman")
pacman::p_load("data.table","dplyr","ggplot2","binhf")
...
#assets coordinates
assets <- 100*data.frame(stdev = unique(sqrt(unlist(lapply(sig, function(x) head(x,2))))), mean = unique(unlist(mu)))
...
assets$correl <- unlist(lapply(sig, last))/sqrt(mapply(prod, lapply(sig, function(x) head(x,2))))

#the resulting curves
curve <- do.call(rbind, pair_tot[-length(pair_tot)])
curve <- curve[order(curve$mean),]
curve_2 <- pair_tot[length(pair_tot)][[1]]
...
ggplot(data =  assets, aes(x = stdev, y = mean)) +
  xlim(c(0.9,1.1)*range(curve_2$stdev)) + ylim(c(0.9,1.1)*range(curve_2$mean)) +
  geom_point(aes(fill = as.factor(correl))) +
  ...
  geom_point(data = assets, aes(stdev, mean), size = 3) +
  annotate(geom = "text", x = assets$stdev,  y = assets$mean, label = LETTERS[seq_along(mu)], 
           fontface = 2, hjust = -0.6, vjust = 1.2) +
  scale_fill_manual(
    values = rep("black", 6), labels = parse(text = sprintf("rho == %s", round(assets$correl,2), drop = F))) +
  # scale_fill_manual(
  #   values = rep("black",6),
